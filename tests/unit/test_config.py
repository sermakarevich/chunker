...
class TestModelProfile:
    def test_has_required_fields(self):
        ...
        assert profile.max_chunk_tokens == 500
        assert profile.context_budget_tokens == 2500
        assert profile.token_factor == 1.3

    def test_gemma_profile(self):
        profile = MODEL_PROFILES["gemma4:12b"]
        assert profile.context_window == 16384
        assert profile.max_chunk_tokens == 400
        assert profile.context_budget_tokens == 2000
...
class TestChunkerConfig:
    ...
    def test_from_model_applies_profile(self):
        config = ChunkerConfig.from_model("gemma4:12b")
    ...
    def test_from_model_unknown_uses_defaults(self):
        ...
        assert config.max_chunk_tokens == 500  # default

    def test_from_model_with_overrides(self):
        config = ChunkerConfig.from_model("gemma4:12b", min_chunk_tokens=200)
        assert config.min_chunk_tokens == 200
        assert config.max_chunk_tokens == 400  # from profile
